...
def read_pointcloud(self, context):
    ...
    if path.split(".")[-1] == "las" or path.split(".")[-1] == "laz":
        with laspy.open(path) as file:
            ...
            #tqdm.tqdm(enumerate(triangles), total=triangle_count, desc="Texturing")
            total_points = h.point_count
            for data_chunk in tqdm.tqdm(file.chunk_iterator(10_000), total=total_points // 10000, desc="Reading Pointcloud"):
